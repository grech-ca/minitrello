...
export const EditMenu = ({ cardId, onClose, isOpen, floatingCard }: EditMenuProps) => {
    ...
    const {
        floatingStyles: menuFloatingStyles, refs: menuRefs, placement: menuPlacement,
    } = useFloating({
    ...
    const cardActions = useMemo<{ icon: IconType; title: string; action: () => void; }[]>(
        ...
        [cardId, location, navigate, onClose, removeCard]
    );
                        ...
                        >
                            {cardActions.map(({ icon: ActionIcon, title, action }, index) => (
                                <Transition.Child
                                    key={title}
                                    as="button"
                                    style={{ '--delay': `${index * 20}ms` } as CSSProperties}
